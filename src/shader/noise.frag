...
void main() {
  ...
  vec2 uv =  v_textureCoords / u_resolution;
  ...
  float red   =   texture2D(  u_sampler,  vec2(uv.x + xOffset -0.5*rgbOffsetOpt,y)).r+staticVal;
  float green =   texture2D(  u_sampler,  vec2(uv.x + xOffset,    y)).g+staticVal;
  float blue  = texture2D(  u_sampler,  vec2(uv.x + xOffset +0.5*rgbOffsetOpt,y)).b+staticVal;
